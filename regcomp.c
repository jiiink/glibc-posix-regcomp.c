...
static void compile_fastmap_for_state(struct re_pattern_buffer *bufp, 
                                      re_dfa_t *dfa, 
                                      void *state, 
                                      char *fastmap)
{
    if (dfa->init_state != state)
        re_compile_fastmap_iter(bufp, state, fastmap);
}
...
re_compile_fastmap (struct re_pattern_buffer *bufp)
{
    ...
    compile_fastmap_for_state(bufp, dfa, dfa->init_state_word, fastmap);
    compile_fastmap_for_state(bufp, dfa, dfa->init_state_nl, fastmap);
    compile_fastmap_for_state(bufp, dfa, dfa->init_state_begbuf, fastmap);
...
static void
process_character_node(re_dfa_t *dfa, regex_t *bufp, Idx node, char *fastmap, bool icase)
{
...
static void
process_simple_bracket_node(re_dfa_t *dfa, Idx node, char *fastmap, bool icase)
{
    int i, ch;
...
#ifdef RE_ENABLE_I18N
static void
handle_collation_elements(re_charset_t *cset, char *fastmap, bool icase)
{
#ifdef _LIBC
...
static void
handle_multibyte_characters(re_charset_t *cset, char *fastmap)
{
    unsigned char c = 0;
    do
    {
...
static void
handle_mbchars(re_charset_t *cset, regex_t *bufp, re_dfa_t *dfa, char *fastmap, bool icase)
{
...
static void
process_complex_bracket_node(re_dfa_t *dfa, regex_t *bufp, Idx node, char *fastmap, bool icase)
{
    re_charset_t *cset = dfa->nodes[node].opr.mbcset;
    
    handle_collation_elements(cset, fastmap, icase);
    ...
    {
        handle_multibyte_characters(cset, fastmap);
    }
    else
    {
        handle_mbchars(cset, bufp, dfa, fastmap, icase);
    }
}
#endif

static bool
is_end_marker_type(re_token_type_t type)
{
...
static void
re_compile_fastmap_iter(regex_t *bufp, const re_dfastate_t *init_state, char *fastmap)
{
    re_dfa_t *dfa = bufp->buffer;
    ...
    for (Idx node_cnt = 0; node_cnt < init_state->nodes.nelem; ++node_cnt)
    {
        ...
        if (type == CHARACTER)
        {
            process_character_node(dfa, bufp, node, fastmap, icase);
        }
        else if (type == SIMPLE_BRACKET)
        {
            process_simple_bracket_node(dfa, node, fastmap, icase);
        }
#ifdef RE_ENABLE_I18N
        else if (type == COMPLEX_BRACKET)
        {
            process_complex_bracket_node(dfa, bufp, node, fastmap, icase);
        }
#endif
        else if (is_end_marker_type(type))
        {
...
{
  reg_errcode_t ret;
  reg_syntax_t syntax = determine_syntax(cflags);

  initialize_preg_buffer(preg);

  if (__glibc_unlikely (allocate_fastmap(preg) != 0))
    return REG_ESPACE;

  syntax = apply_case_flag(syntax, cflags);
  configure_newline_behavior(preg, cflags, &syntax);
  
  preg->no_sub = !!(cflags & REG_NOSUB);
  preg->translate = NULL;

  ret = re_compile_internal (preg, pattern, strlen (pattern), syntax);
  ret = normalize_paren_error(ret);

  finalize_compilation(preg, ret);

  return (int) ret;
}

static reg_syntax_t determine_syntax(int cflags)
{
  return (cflags & REG_EXTENDED) ? RE_SYNTAX_POSIX_EXTENDED : RE_SYNTAX_POSIX_BASIC;
}

static void initialize_preg_buffer(regex_t *preg)
{
  preg->buffer = NULL;
  preg->allocated = 0;
  preg->used = 0;
...
static int allocate_fastmap(regex_t *preg)
{
  preg->fastmap = re_malloc (char, SBC_MAX);
  return (preg->fastmap == NULL) ? 1 : 0;
}

static reg_syntax_t apply_case_flag(reg_syntax_t syntax, int cflags)
{
  return syntax | ((cflags & REG_ICASE) ? RE_ICASE : 0);
}

static void configure_newline_behavior(regex_t *preg, int cflags, reg_syntax_t *syntax)
{
...
static reg_errcode_t normalize_paren_error(reg_errcode_t ret)
{
  return (ret == REG_ERPAREN) ? REG_EPAREN : ret;
}

static void finalize_compilation(regex_t *preg, reg_errcode_t ret)
{
